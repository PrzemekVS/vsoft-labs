...
// <snippet_handlers>
// <snippet_getalltodos>
static async Task<IResult> GetAllTodos(TodoDb db, ServiceBusService serviceBus)
{
    return TypedResults.Ok(await db.Todos.Select(x => new TodoItemDTO(x)).ToArrayAsync());
}
// </snippet_getalltodos>

static async Task<IResult> GetCompleteTodos(TodoDb db, ServiceBusService serviceBus) {
...
static async Task<IResult> DeleteTodo(int id, TodoDb db, ServiceBusService serviceBus)
{
    if (await db.Todos.FindAsync(id) is Todo todo)
    {
        ...
        await db.SaveChangesAsync();
        await serviceBus.SendMessageAsync(new TodoEvent("TodoDeleted", new TodoItemDTO
        {
            Id = id
        }));
        return TypedResults.NoContent();
